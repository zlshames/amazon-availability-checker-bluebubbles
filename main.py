...
AMAZON_PRODUCT_NAME = os.getenv('AMAZON_PRODUCT_NAME')
AMAZON_PRODUCT_ID = os.getenv('AMAZON_PRODUCT_ID')
...
def is_available():
    ...
    availability_string = str(soup.find('div', id='availability-string'))
...
if __name__ == '__main__':
    logger.info(f'Checking Amazon product availability for product ID: {AMAZON_PRODUCT_ID}')
    available = is_available()
    if available:
        logger.info(f'Product is already available! Waiting {INTERVAL} seconds before checking again...')
        # waiting_until_unavailable = True
